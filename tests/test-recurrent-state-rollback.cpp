...
static bool test_fragmented_on_device_fallback(
    ...
    if (ok) {
        ...
        if (ckpt.data_tgt_on_device) {
            // Some recurrent layouts compact the surviving sequences into one
            // device range.  That is valid and does not require host fallback.
            fprintf(stderr, "%s : checkpoint remained device-contiguous\n", __func__);
        } else {
            fprintf(stderr, "%s : fragmented checkpoint fell back to host storage\n", __func__);
        }
    }
    if (ok) {
        // load_tgt() must use the actual storage mode recorded when the
        // checkpoint was saved, whether the layout stayed contiguous or fell
        // back to host storage.
...
static bool test_preallocated_device_checkpoints(
    ...
    if (!ok) {
        // Architectures without recurrent tensor rows have no device layout to
        // reserve.  The server handles this by using host checkpoints.
        fprintf(stderr, "%s : no device checkpoint layout; using host checkpoints\n", __func__);
        llama_free(ctx);
        return true;
    }
